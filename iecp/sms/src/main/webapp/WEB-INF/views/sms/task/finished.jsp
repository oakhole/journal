...
<body>
	<div class="content-box role">
		...
		<div class="content-box-header">
			<h3 style="cursor: s-resize;">发送列表</h3>
			<div class="clear"></div>
		</div>
		<div class="content-box-content">
			<div class="searchgrid">
				<form method="post" action="sms-send-manage!findAllSmsSend"
					name="Form" id="Form">
					<input type="hidden" value="2" name="sendStatus" /> <label>发送账号：</label><input
				...
				<table width="100%">
					<thead>
						<tr>
							...
							<th>发送状态</th>
							<th>下载</th>
							<c:if test="${sessionScope.GLOBALAPPUSER.isStatus==1}">
								<th>操作</th>
							</c:if>
						</tr>
					</thead>
						...
						<c:if test="${page.totalCount > 0}">
							<c:forEach var="smsSend" items="${page.result}"
								...
								<tr class="alt-row">
									...
									<td><c:if test="${smsSend.sendStatus==0}">
											发送中
										</c:if> <c:if test="${smsSend.sendStatus==1}">
											发送中
										</c:if> <c:if test="${smsSend.sendStatus==2}">
											<font color="green">发送成功</font>
										</c:if> <c:if test="${smsSend.sendStatus==3}">
											<font color="red">发送失败</font>
										</c:if></td>
									<td><a
										href="sms-send-manage!downLoadPhoneNumber?sendFilePath=${smsSend.sendFilePath}">下载号码</a>
									</td>
									<td><c:if test="${sessionScope.GLOBALAPPUSER.isStatus==1}">
											<c:if test="${smsSend.sendStatus==2}">
												<a href="sms-manage!findAllSms?sendId=${smsSend.id}">详情</a>
											</c:if>
											<c:if test="${sessionScope.GLOBALAPPUSER.userType==0}">
												| <a
													href="sms-send-manage!alterSmsSend?smsSendId=${smsSend.id}">修改</a>
												| <a
								...
								<div class="pagination">
									<form action="?" name="formpage" method="post">
										<a>共有 <b>${page.totalCount}/${page.pageCount}页</b> </a> <a
											class="number"
											<c:if test="${page.currentPage > 1}">href="sms-send-manage!findAllSmsSend?sendStatus=2&account=${account}&content=${content}&starttime=${startTime}&endtime=${endTime}&taskId=${taskId}&currentPage=${page.currentPage-1}"</c:if>>上一页</a>
										<c:forEach var="pagenum" items="${page.pageNum}"
											...
											<c:if test="${pagenum!=page.currentPage}">
												<a class="number"
													href="sms-send-manage!findAllSmsSend?sendStatus=2&account=${account}&content=${content}&starttime=${startTime}&endtime=${endTime}&taskId=${taskId}&currentPage=${pagenum}">${pagenum}</a>
											</c:if>
										</c:forEach>
										<a
											<c:if test="${page.currentPage < page.pageCount}">href="sms-send-manage!findAllSmsSend?sendStatus=2&account=${account}&content=${content}&starttime=${startTime}&endtime=${endTime}&taskId=${taskId}&currentPage=${page.currentPage+1}"</c:if>
											...
											type="hidden" name="sendStatus" value="2" /><input
