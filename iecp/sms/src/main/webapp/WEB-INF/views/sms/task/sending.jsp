...
<body>
	<div class="content-box role">
		...
		<div class="content-box-header">
			<h3 style="cursor: s-resize;">正在发送</h3>
			<div class="clear"></div>
		</div>
		<div class="content-box-content">
			<div class="searchgrid">
				<form method="post" action="sms-send-manage!findAllSmsSend"
					name="Form" id="Form">
					<input type="hidden" value="1" name="sendStatus" /> <label>发送账号：</label><input
						...
						<c:if test="${page.totalCount > 0}">
							<c:forEach var="smsSend" items="${page.result}"
								...
								<tr class="alt-row">
									...
									<td><c:if test="${sessionScope.GLOBALAPPUSER.userType==0}">
											<a
												href="sms-send-manage!changeSendStatus?smsSendIds=${smsSend.id}&sendStatus=0"><font
												color="red">驳回</font> </a>
										</c:if>
									</td>
								</tr>
								...
								<div class="pagination">
									<form action="?" name="formpage" method="post">
										<a>共有 <b>${page.totalCount}/${page.pageCount}页</b> </a> <a
											class="number"
											<c:if test="${currentPage > 1}">href="sms-send-manage!findAllSmsSend?sendStatus=1&account=${account}&content=${content}&starttime=${startTime}&endtime=${endTime}&taskId=${taskId}&currentPage=${page.currentPage-1}"</c:if>>上一页</a>
										<c:forEach var="pagenum" items="${page.pageNum}"
											...
											<c:if test="${pagenum!=page.currentPage}">
												<a class="number"
													href="sms-send-manage!findAllSmsSend?sendStatus=1&account=${account}&content=${content}&starttime=${startTime}&endtime=${endTime}&taskId=${taskId}&currentPage=${pagenum}">${pagenum}</a>
											</c:if>
										</c:forEach>
										<a
											<c:if test="${currentPage < page.pageCount}">href="sms-send-manage!findAllSmsSend?sendStatus=1&account=${account}&content=${content}&starttime=${startTime}&endtime=${endTime}&taskId=${taskId}&currentPage=${page.currentPage+1}"</c:if>
											...
											type="hidden" name="sendStatus" value="1" /><input
