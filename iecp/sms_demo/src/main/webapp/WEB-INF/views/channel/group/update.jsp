		...
		<div class="content-box-content">
			<div style="display: block;" class="tab-content default-tab"
				...
				<form method="post" action="channel-manage!saveChannelGroup"
					...
					<input name="flag" value="1" type="hidden" /> <input
						name="channelGroupId" value="${channelGroup.id}" type="hidden" />
						...
						<tbody>
							<tr class="alt-row">
								...
								<td align="left" height="25" width="*"><input class=""
									...
									nullable="false" errmsg="通道组名称" valuetype="STRING" type="text"
									value="${channelGroup.name}"><font color="red">*</font>
								</td>
							</tr>
							<tr class="alt-row">
								<td class="left" align="right" width="30%" height="25">移动通道:</td>
								<td align="left" width="*" height="25"><select
									name="yidongChannelId" id="channelId">
										<option value="0">请选择通道</option>
										<c:forEach var="yidongChannel" items="${yidongChannels}"
											varStatus="status">
											<c:if
												test="${channelGroup.yidongChannelId==yidongChannel.id}">
												<option value="${yidongChannel.id}" selected="selected">${yidongChannel.name}</option>
											</c:if>
											<c:if
												test="${channelGroup.yidongChannelId!=yidongChannel.id}">
							...
							<tr class="alt-row">
								<td class="left" align="right" width="30%" height="25">联通通道:</td>
								<td align="left" width="*" height="25"><select
									name="liantongChannelId" id="channelId">
										<option value="0">请选择通道</option>
										<c:forEach var="liantongChannel" items="${liantongChannels}"
											varStatus="status">
											<c:if
												test="${channelGroup.liantongChannelId==liantongChannel.id}">
												<option value="${liantongChannel.id}" selected="selected">${liantongChannel.name}</option>
											</c:if>
											<c:if
												test="${channelGroup.liantongChannelId!=liantongChannel.id}">
							...
							<tr class="alt-row">
								<td class="left" align="right" width="30%" height="25">电信通道:</td>
								<td align="left" width="*" height="25"><select
									name="dianxinChannelId" id="channelId">
										<option value="0">请选择通道</option>
										<c:forEach var="dianxinChannel" items="${dianxinChannels}"
											varStatus="status">
											<c:if
												test="${channelGroup.dianxinChannelId==dianxinChannel.id}">
												<option value="${dianxinChannel.id}" selected="selected">${dianxinChannel.name}</option>
											</c:if>
											<c:if
												test="${channelGroup.dianxinChannelId!=dianxinChannel.id}">
