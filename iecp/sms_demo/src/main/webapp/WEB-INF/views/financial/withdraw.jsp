	...
	<div class="content-box role">
		...
		<div class="content-box-content">
			<div class="searchgrid">
				<form method="post" action="financial-manage!withdraw" name="Form"
					id="Form1">
					<label>消费帐号：</label> <input id="account" style="width:85px"
			...
			<div style="display: block;" class="tab-content default-tab">
				...
				<table>
					<thead>
						<tr>
							<th style="padding-left:5px;border-left:1px solid #C1DAD7">消费帐号</th>
							<th>操作帐号</th>
							<th>消费数量</th>
							<th>扣费日期</th>
							<!--<th>充值类型</th>-->
						...
						<c:if test="${page.totalCount > 0}">
							<c:forEach var="financial" items="${page.result}"
								...
								<tr class="alt-row">
									...
									<c:if test="${financial.actMethod == 1}">
										<td title="${financial.memo}">短信扣费</td>
									</c:if>
									<c:if test="${financial.actMethod == 2}">
										<td title="${financial.memo}">人工扣费</td>
									</c:if>
								</tr>
							</c:forEach>
						</c:if>
								...
								<div class="pagination">
									<form action="?" name="formpage" method="post">
										<a>共有 <b>${page.totalCount}条/${page.pageCount}页</b> </a> <a
											class="number"
											<c:if test="${page.currentPage > 0}"> href="financial-manage!withdraw?account=${account}&startTime=${startTIme}&endTime=${endTime}&currentPage=${page.currentPage-1}"</c:if>>上一页</a>
										<c:forEach var="pagenum" items="${page.pageNum}"
											...
											<c:if test="${pagenum != page.currentPage}">
												<a class="number"
													href="financial-manage!withdraw?account=${account}&startTime=${startTIme}&endTime=${endTime}&currentPage=${pagenum}">${pagenum}</a>
											</c:if>
										</c:forEach>
										<a class="number"
											<c:if test="${page.currentPage < page.pageCount}"> href="financial-manage!withdraw?account=${account}&startTime=${startTIme}&endTime=${endTime}&currentPage=${page.currentPage+1}"</c:if>>下一页</a>
